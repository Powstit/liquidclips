...
interface CrewMatchRow {
  ...
  // 2026-07-07 · HQ gap contract. The opportunity is the pitch.
...
/** Format a low/high range · Social Blade style. Falls back to single
 *  number when the band isn't computed yet (early enrichment window). */
function fmtRange(low: number | null, mid: number | null, high: number | null): string {
  if (low && high && high > low) {
    return `${fmtDollars(low)}–${fmtDollars(high)}`;
  }
  return fmtDollars(mid ?? 0);
}

/** Turn "tiktok,instagram_reels" into "TikTok + Reels" for the pitch. */
function fmtAbsent(csv: string): string {
  ...
  const parts = csv.split(/[,\s]+/).filter(Boolean).map((p) => map[p] ?? p);
  if (parts.length === 0) return "other platforms";
  ...
  return (
    <section className="lc-crew" aria-label="Crew match tool">
      ...
      <p className="lc-crew-sub">
        ...
        We&rsquo;ll show which of them are already on our list — and how much money
        they&rsquo;re leaving on the table by not clipping. Invite them, keep 50% of
        their subscription forever.
      </p>
        ...
        <>
          <div className="lc-crew-result-head">
            ...
            <div className="lc-crew-stat-callout">
              <div className="lc-crew-stat-eb">Total money they're leaving on the table</div>
              <div className="lc-crew-stat-num accent">
                {fmtDollars(result.matched.reduce((sum, r) => sum + (r.estimated_opportunity_cents ?? 0), 0))}
                <span className="lc-crew-stat-num-mo">/mo</span>
              </div>
              <div className="lc-crew-stat-note">
                Your 50% cut if they convert · {fmtDollars(result.earning_potential_cents)}/mo
              </div>
            </div>
          </div>
              ...
              {result.matched.map((r) => (
                <li key={r.email} className="lc-crew-row">
                  <div className="lc-crew-row-body">
                    <div className="lc-crew-row-handle">
                      @{r.handle}
                      {r.earnings_verified_by_owner && (
                        <span className="lc-crew-row-verified" title="Owner-verified earnings">✓</span>
                      )}
                    </div>
                    ...
                    {/* THE PITCH · loss aversion. Lead with the gap. */}
                    {r.estimated_opportunity_cents ? (
                      <div className="lc-crew-row-gap">
                        Leaving <span className="lc-crew-row-gap-num">{fmtDollars(r.estimated_opportunity_cents)}/mo</span>
                        {r.absent_platforms && (
                          <span className="lc-crew-row-absent"> on {fmtAbsent(r.absent_platforms)}</span>
                        )}
                      </div>
                    ) : null}
                    {/* Vanity setup · smaller, gray. Range if we have it. */}
                    {r.estimated_monthly_earnings_cents ? (
                      <div className="lc-crew-row-earnings">
                        Makes {fmtRange(r.earnings_low_cents, r.estimated_monthly_earnings_cents, r.earnings_high_cents)}/mo now
                        <span className="lc-crew-row-cut"> · your 50% = {fmtDollars(r.your_50pct_cents)}/mo</span>
                      </div>
                    ) : !r.estimated_opportunity_cents ? (
                      <div className="lc-crew-row-earnings muted">Earning data · <em>not measured yet</em></div>
                    ) : null}
                  </div>
                  <button
                    type="button"
                    className="lc-crew-invite"
                    onClick={() => void onInvite(r)}
...
.lc-crew-stat-callout .lc-crew-stat-num { font-size: 32px; font-weight: 800; }
.lc-crew-stat-num-mo { font-size: 14px; font-weight: 500; margin-left: 4px; opacity: 0.72; }
.lc-crew-stat-note {
...
.lc-crew-row-earnings {
  margin-top: 4px; font-size: 12px; color: rgba(255, 255, 255, 0.62);
}
.lc-crew-row-earnings.muted { color: rgba(255, 255, 255, 0.42); }
.lc-crew-row-cut { color: #ff66b8; font-weight: 600; }
.lc-crew-row-verified {
  margin-left: 6px; color: #22c55e; font-size: 12px; font-weight: 700;
}
.lc-crew-row-gap {
...
.lc-crew-row-gap-num {
  color: #ff1a8c; font-weight: 800; font-size: 15px;
}
.lc-crew-row-absent {
