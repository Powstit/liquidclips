...
// and public/brand/intro/*.mp4. Don't remove the WebKit autoplay fallback,
    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="absolute right-6 top-6 z-20 rounded-full border border-fuchsia bg-paper/45 px-3 py-1.5 font-sans text-[11px] font-semibold uppercase tracking-[0.12em] text-fuchsia shadow-[var(--glow-sm)] backdrop-blur-sm transition-all hover:bg-fuchsia hover:text-white focus:outline-none focus:ring-2 focus:ring-fuchsia focus:ring-offset-2 focus:ring-offset-black"
        >
          Skip intro
        </button>
    ...
    <div
      ...
      style={
        stage === "game"
          ...
          : { backgroundImage: `url(/brand/intro/closing-still.png)`, backgroundSize: "cover", backgroundPosition: "center" }
      }
    >
      ...
      {/* Stage 2 (loading) = brand moment with loading bar.
          Stage 3 (game) = SplashGame ecosystem — grid + HUD counters +
          leaderboard panel + SplashGame inside. Logo tucked top-left
          for game stage (was centered xxl during loading). */}
      {stage === "game" && onContinue ? (
        <>
          <SplashGrid />
          <div className="splash-mark-anim absolute left-6 top-5 z-30 animate-[splash-mark-in_0.6s_ease-out]">
            <Logo size="lg" showVersion={false} />
          </div>
