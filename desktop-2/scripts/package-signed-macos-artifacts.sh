#!/usr/bin/env bash
# package-signed-macos-artifacts.sh — rebuild release artefacts from a
# signed .app.
#
# Run AFTER sign-clean-macos-app.sh. This guarantees the DMG (and the
# updater tarball if `TAURI_SIGNING_PRIVATE_KEY` is set) contains the
# repaired/signed app, not a stale bundle emitted before post-build signing.
#
# Usage:
#   ./scripts/package-signed-macos-artifacts.sh /path/to/Liquid\ Clips.app [arch]
#
# Ported verbatim from legacy desktop/scripts/package-signed-macos-artifacts.sh
# · uses tauri.conf.json for productName + version · arch defaults to
# `uname -m` so a local rehearsal just produces the current-arch DMG.
...
if [ -n "${TAURI_SIGNING_PRIVATE_KEY:-}" ]; then
  ...
  echo "(skip) TAURI_SIGNING_PRIVATE_KEY not set; updater .sig not generated (P1-4-d will wire this)"
fi
...
# Codesign the rebuilt DMG container itself. The .app inside is already
# signed (by sign-clean-macos-app.sh upstream), but `hdiutil create`
# produces a fresh unsigned DMG. Without this step `spctl --assess --type
# install` would fail with `source=no usable signature` after notarisation.
...
echo "✓ DMG ready for notarisation: $DMG_PATH"
