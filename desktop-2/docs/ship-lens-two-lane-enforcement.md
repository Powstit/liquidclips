...
| 5b | (disabled — TODO) | money surfaces       | `SectionWithFallback` mount enforcement. Enable in a follow-up sprint that lands the wrap around every money surface. |
...
3, 5, and 5b. Rule 4 still applies (no `*_rendered` telemetry
...
Rule 5b · `SectionWithFallback` mount enforcement — enable once the
serial pass mounts `SectionWithFallback` around every money surface in
a follow-up sprint. The disabled placeholder is in the JSON so the
enable-day diff is a one-line flag flip, not a schema change.
