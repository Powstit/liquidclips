...
>
> Currently active gates: IG-001 (import pipeline), IG-002 (sidecar RPC), IG-003 (cinematic intro), IG-004 (auth + activation), IG-005 (workspace UI design), IG-006 (cockpit handoff contracts), IG-007 (ClipCard structure), IG-008 (cockpit room scrollability + BottomCockpit clearance), IG-009 (cloud release flow — `scripts/cloud-ship.sh`), IG-010 (v0.8.0 non-blocking architecture — `sidecar.py` METHODS dispatcher + `sidecar.ts` 10 background bridges + 4 event listener pairs + `useGlobalBakeEvents` on-mount attach), IG-011 (webview room height cascade — RoomShell `align="stretch"` for native-webview rooms), IG-012 (brand kit single source of truth — `src/index.css` canonical tokens mirrored in `docs/demo*.html`, enforced by `scripts/brand-kit-drift-check.sh` in pre-commit), IG-013 (Apple notarisation chain — `.github/workflows/release.yml` + `scripts/notarize.sh` + 5 GH secrets; canonical `xcrun notarytool submit --wait` + `xcrun stapler staple`), IG-014 (auth-keychain invariant — `src/lib/authStorage.ts` central module + `app.liquidclips.auth.v1` namespace + `scripts/assert-no-passive-keychain.sh` pre-commit + `tests/no-passive-keychain.test.mjs` + canonical doc `docs/auth-keychain-invariant.md`). Full registry: `docs/IRON_GATES.md`.
