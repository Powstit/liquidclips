            ...
            {([1, 2, 3, 4] as Weeks[]).map((w) => (
              <button
                ...
                className={`rounded-full px-4 py-1.5 font-mono text-[12px] uppercase tracking-[0.08em] transition-colors ${
                  weeks === w ? "bg-fuchsia text-ink" : "border border-line text-text-secondary hover:border-fuchsia hover:text-ink"
                }`}
              >
                {w}w
              </button>
        ...
        <footer className="flex items-center justify-between border-t border-line px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={confirm}
              disabled={!slots || busy}
              className="rounded-full bg-fuchsia px-5 py-2.5 font-sans text-[14px] font-medium text-ink hover:bg-fuchsia-bright hover:shadow-[0_10px_30px_rgba(255,26,140,0.3)] disabled:opacity-50"
            >
