/**
 * Wallet API client · 2026-06-25 · talks to junior-backend /me/wallet/summary.
 *
 * Ported from desktop-2/src/lib/wallet.ts. Adapted for the LEGACY desktop/
 * shell:
 *   • JWT pulled from authStorage.ts via getCachedLicenseJwt() — passive,
 *     never touches Keychain (IG-014 invariant — see authStorage.ts).
 *   • backendUrl() reuses the same VITE_BACKEND_URL → https://api.liquidclips.app
 *     fallback as the canonical lib/backend.ts.
 *
 ...
 * the withdraw button when false · the desktop/ shell defers the actual
 * `claimCarrot()` wire-up to the v2 sidecar sprint — see WalletPanel.tsx
 * for the temporary "Withdrawals coming soon" copy that ships in v0.7.79.
 */
...
/**
 * Fetch the wallet summary. Returns null on:
 *   • no cached JWT (caller renders sign-in branch)
 *   • non-2xx (backend offline / 401 / 5xx)
 *   • network error / timeout / abort
 *
 * Caller decides what to paint when null — empty state or "wallet
 * briefly out of reach" depending on context.
 *
 * NEVER triggers a Keychain prompt — uses getCachedLicenseJwt() per the
 * IG-014 auth-keychain invariant. If the cache is empty, the request is
 * still issued unauthenticated and the backend returns 401, which we
 * surface as null.
 */
