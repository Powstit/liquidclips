...
// v0.7.58 P0 — All JWT cache state lives in `src/lib/authStorage.ts`. This
// module re-exports the safe accessors for back-compat so existing imports
// keep compiling, but every call goes through the central module that
// enforces the auth-keychain invariant. See `docs/auth-keychain-invariant.md`.
import {
  getCachedLicenseJwt,
...
async function authedFetch(
  ...
  // v0.7.58 P0 — auth-keychain invariant: no caller is permitted to unlock
  // the OS Keychain from a backend fetch. Reads come from the in-memory
  // cache (primed by explicit auth actions only). Cache miss → throw
  // UnauthorizedError so the caller renders the reconnect UI.
  const jwt = maybeJwt ?? getCachedLicenseJwt();
  if (!jwt) {
    throw new UnauthorizedError(RECONNECT_PROMPT_COPY);
  }
...
export async function maybeCheckQuota(): Promise<QuotaCheck> {
  // v0.7.58 P0 — auth-keychain invariant. Quota check is NOT one of the
  // five explicit auth actions; it runs from the Export click chain and
  // must use the in-memory cache only. Cache miss surfaces upstream as the
  // reconnect prompt; the pipeline still proceeds leniently per the
  // existing "unknown" contract.
