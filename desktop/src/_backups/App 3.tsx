...
// v0.6.5 — Single unified entry box (paste/browse/drop in one surface)
// replaces the v0.6.3 DropZone. Lives ABOVE the dashboard so the user
// never scrolls to take an action.
...
import { sidecar, visibleStagesFor, pipelineStagesFor, BACKGROUND_STAGES, onIngestProgress, onLiftProgress, type BountyContext, type IngestProgress, type Intent, type LiftProgress, type LiftTranscriptResult, type Project, type StageName } from "./lib/sidecar";
...
export default function App() {
  ...
  async function runRemainingStages(initial: Project) {
    ...
    // v0.6.8 — Background stages. Thumbnails (5-20s tail) run AFTER results
    // are visible so the Opus-vs-Liquid timer measures time-to-first-clip,
    // not time-to-thumbnail. Fire-and-forget: a failure here only means cards
    // fall back to the video-element preview (already the default render).
  ...
  return (
    <MainShell>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center justify-end gap-4 border-b border-line px-6 py-4">
      ...
      <main className="flex flex-1 items-stretch justify-center overflow-y-auto px-6 py-10">
        ...
        {view.kind === "empty" && bootChecked && (
          <div className="flex w-full max-w-[960px] flex-col items-stretch gap-5">
            {/* v0.6.5 — Single unified entry surface (paste / browse /
                drop). Sits ABOVE the dashboard so users never have to
                scroll before taking an action. Daniel locked direction:
                "one box should do everything — what you do leads to
                whatever". The CTA routes by content: empty URL → file
                picker, URL pasted → ingest+cut or lift-transcript per
                the Output mode toggle. */}
            <UnifiedDropZone
              onPickFile={pickFile}
              onPasteUrl={onPasteUrl}
              onLiftTranscript={(url) => void onLiftTranscript(url)}
              onImportReadyClips={(project) => setView({ kind: "results", project })}
              remainingExports={remainingExports}
            />

            {/* v0.6.4 — Stickiness Dashboard sits BELOW the entry box.
                Rank + affiliate + live campaigns + active clips +
                leaderboard. Drop CTA inside the dashboard is retired
                since the unified box above already covers entry. */}
            <WorkspaceDashboard
              onOpenDrop={() => void pickFile("")}
              onOpenCampaign={(bountyId) => {
                void bountyId;
                setView({ kind: "earn" });
              }}
              onOpenEarn={() => setView({ kind: "earn" })}
              onOpenAffiliate={() => setView({ kind: "earn" })}
            />
          </div>
        )}
