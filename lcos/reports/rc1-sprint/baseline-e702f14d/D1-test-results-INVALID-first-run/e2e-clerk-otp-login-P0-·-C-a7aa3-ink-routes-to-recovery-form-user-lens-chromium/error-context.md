...
- Name: e2e/clerk-otp-login.spec.ts >> P0 · Clerk OTP login surface >> LC-ID fallback link routes to recovery form
- Location: tests/e2e/clerk-otp-login.spec.ts:83:3
...
Locator:  getByTestId('welcome-existing').first()
Expected: visible
Received: hidden
Timeout:  8000ms
...
Call log:
  ...
  - waiting for getByTestId('welcome-existing').first()
    18 × locator resolved to <button type="button" data-testid="welcome-existing" class="lc-login-cta lc-login-cta--primary">…</button>
