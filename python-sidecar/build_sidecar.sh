...
COLLECT_ALL_ARGS=(
  ...
  # bundle cv2's data dir by default — the .xml files get stripped
  # and cv2 raises (-215:Assertion failed) !empty() at runtime.
  # --collect-data cv2 ships cv2/data/*.xml (~1.3 MB) so the cascade
  # loader resolves inside the bundle. Binaries are already pulled
  # in via the opencv-python wheel; this only adds the data files.
  --collect-data cv2
)
...
# ── deep adhoc-sign every Mach-O in the bundle ──────────────────────────
# BUG-016b · PyInstaller --onedir only adhoc-signs the entry exe. The
# 150+ Mach-O files inside _internal/ (Python framework, bin/ffmpeg,
# bin/ffprobe, bin/junior-face-detect, plus ~155 .dylib/.so wheels) are
# left unsigned. On macOS 26+ with stricter Hardened Runtime evaluation,
# dyld rejects the load chain with SIGKILL (Code Signature Invalid,
# codeSigningTrustLevel=4294967295) when Tauri spawns the sidecar from
# a freshly-built bundle. Smoke-testing via Terminal can mask this
# (responsible-process trust) but Tauri's spawn path hits it every time.
#
# Adhoc-signing every Mach-O in the bundle makes it a coherent unit that
# dyld accepts. Production CI later re-signs the wrapping .app with the
# Developer ID + notarisation; this adhoc pass is the dev/local invariant
# that lets every rebuild boot under Tauri without manual intervention.
step "Adhoc-sign every Mach-O in the bundle (BUG-016b)"
MACHO_LIST="$(find "$BIN_DIR" -type f -perm +111 -exec file {} + 2>/dev/null \
  | grep "Mach-O" | cut -d: -f1)"
MACHO_COUNT=$(printf "%s\n" "$MACHO_LIST" | grep -c . || true)
if [ "$MACHO_COUNT" -eq 0 ]; then
  fail "found no Mach-O files in $BIN_DIR — bundle layout changed?"
fi
printf "%s\n" "$MACHO_LIST" | xargs -I{} codesign --force --sign - {} 2>/dev/null
# Re-sign the entry binary last so its signature's load-chain captures
# the now-signed internal targets.
codesign --force --sign - "$BIN_PATH"
codesign --verify --deep --strict "$BIN_PATH" 2>/dev/null \
  || fail "deep-sign verify failed for $BIN_PATH"
ok "Signed $MACHO_COUNT Mach-O files + verified entry binary (adhoc)"
...
# resource directory. Codesign signatures live in the Mach-O __LINKEDIT
# segment, so `cp -R` preserves them in the mirror.
