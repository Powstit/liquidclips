...
VERSION = "2.3.10"  # tracked to desktop app version — surfaces in startup log + method_ping
...
def method_start_run(params: dict[str, Any]) -> dict[str, Any]:
    ...
    # BUG-017 P2 — optional user-selected target clip count (10 / 30 / 100).
    # `None` keeps the legacy adaptive-prompt behaviour. Anything outside the
    # [1, 100] sanity range is ignored rather than rejected so the new param
    # never breaks an in-flight client.
    clip_count_raw = params.get("clip_count")
...
def method_ingest_url(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if intent not in ("clips", "youtube", "both"):
        raise ValueError("intent must be one of: clips, youtube, both")
    # BUG-017 P2 — same shape as method_start_run. `None` keeps the legacy
    # adaptive prompt; valid range is 1..100 (UI ships 10 / 30 / 100).
    clip_count_raw = params.get("clip_count")
    clip_count: int | None = None
    if isinstance(clip_count_raw, int) and 1 <= clip_count_raw <= 100:
        clip_count = clip_count_raw
