...
| Completed visual inspected/approved | DONE | Baseline `tests/visual/baselines/workstation-completed-path.png` regenerated after the workstation-compression sprint; the change is intentional (row-gap 140px, aspect 4:5, filter chips inline). Both targeted runs match. |
| Targeted workstation suite passes twice | DONE | Run 1: 7/7 pass in 29.3s. Run 2: 7/7 pass in 24.6s. C2 added an 8th test (skipped gracefully — see next row). |
| Zero-candidate recovery verified | PARTIAL | Effect + explicit `[data-testid="ws-zero-candidates"]` empty-results panel + retry button + CSS shipped (`Workstation.tsx:76-83`, `Workstation.css:212-259`). Playwright test skipped in run because harness doesn't inject a hydrated-with-empty-clips project yet — needs a `sidecar.getProject` mock returning `clips: []` to become a hard-pass. |
| Hydration failure/retry verified | PENDING | Phase C3 pending. |
| Legacy/partial clip payload verified | PENDING | Phase C4 pending. |
| Preview container responsiveness verified | PENDING | Phase C5 pending. |
| Dock responsiveness verified | PENDING | Phase C6 pending. |
| Resume ordering/race verified | PENDING | Phase C7 pending. |
| Package/Tauri/Cargo versions aligned | DONE | Cargo.toml bumped `2.2.0 → 2.2.18` (`src-tauri/Cargo.toml:3`). `assert-shell-contracts.sh` extended with "Version alignment (Phase C1)" section that reports drift without mutating files. Guard: 107 pass / 0 fail. |
| `npm run build` | PENDING | Phase D. |
| `npm run guard` | DONE (partial) | 107/0 after C1 extension. Full re-run in Phase D. |
| `cargo check` | PENDING | Phase D. |
| Full Playwright suite | PENDING | Phase D. |
| `npm run test:invariant` | PENDING | Phase D. |
