"""Liquid Lift free-tier watermark detector (sprint #14c).

Free-tier Liquid Lift exports carry a bright fuchsia "LIQUID LIFT" watermark
positioned middle-right with a slight x-oscillation. The watermark IS the
conversion engine for the Minecraft Story Clip Challenge: submitted clips
must be clean, so users who want to qualify for rewards have to upgrade.
...
WATERMARK_FILTER constant):
  • Color: fuchsia, RGB (255, 26, 140) ≈ #FF1A8C — the Liquid brand fuchsia
  • Position: vertical center, right edge (middle-right at ~80-95% x, 45-55% y)
  • Text: "LIQUID LIFT"
  • Size: 48px bold sans
...
Detection method:
  ...
  • For each frame: HSV-convert, mask the fuchsia color range, count pixels
    in the target region. If >1.5% of pixels in the right-strip match fuchsia,
    watermark detected.
  • False-positive risk: brands with fuchsia logos (e.g. Lyft pink, T-Mobile
    magenta). Mitigated by requiring the pixels to form a horizontal text-like
    shape in the right-strip — not blob-distributed.
...
# Region of interest: right-strip middle. Watermark sits at ~80-95% x, 45-55% y.
# Pad generously so cropping/letterboxing doesn't push it out of the ROI.
_ROI_X_START_FRAC = 0.70
_ROI_X_END_FRAC = 0.99
_ROI_Y_START_FRAC = 0.30
_ROI_Y_END_FRAC = 0.70
