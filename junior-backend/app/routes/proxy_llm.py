...
_QUOTAS_BY_TIER = {
    ...
    # launch cohort. 400k tokens/mo proportional to Pro's 2M-per-$10 rate
    # at the same $2/mo pilot budget as the primary Anthropic path
    # (proxy_anthropic.py's _QUOTA_CENTS_BY_TIER). This is the
    # legacy/secondary OpenAI-hosted path — hosted_anthropic is tried
    # first, this only fires as the real fallback when Anthropic is down.
    #
    # 2026-08-07 · extended to every paid tier — no tier ever asks a user
    # for their own AI key. Solo's 1M tokens/mo mirrors the same $5-vs-$10
    # ratio used for its Anthropic quota.
    "free": 400_000,
    "solo": 1_000_000,
    "pro": 2_000_000,
    "agency": 8_000_000,
