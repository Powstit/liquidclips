...
const JOURNEYS: Journey[] = [
  // ── Identity + access (12) ──
  { id: "id-01", cluster: "identity", name: "First-launch intro splash → free-tier shell", status: "wired", citation: "desktop-2/src/App.tsx:73", note: "IntroSplash gate + localStorage/keychain JWT cold-boot." },
  { id: "id-02", cluster: "identity", name: "Sign in pill → Whop checkout → JWT deep link", status: "wired", citation: "desktop-2/src/design-os/components/TopHud.tsx:169", endpoint: "GET /whop/checkout-success", note: "Native browser → Whop hosted → 302 to backend → apply_membership_tier → activation-fallback HTML → deep link → keychain." },
  ...
  { id: "id-06", cluster: "identity", name: "Settings → Connections (Ayrshare / Whop / Gmail / YouTube)", status: "wired", citation: "junior-backend/app/routes/social.py:84", endpoint: "POST /social/connect", note: "Profile-Key paste + OAuth flow → social_connections row." },
  { id: "id-07", cluster: "identity", name: "Settings → Profile + unified handle", status: "wired", citation: "junior-backend/app/routes/handle.py:38", endpoint: "POST /me/handle", note: "Handle edit + avatar + tier display from /me." },
  { id: "id-08", cluster: "identity", name: "Settings → Notifications", status: "wired", citation: "junior-backend/app/routes/notifications.py:47", endpoint: "GET /notifications", note: "Inbox list from backend." },
  ...
  { id: "mo-14", cluster: "money", name: "Stripe Connect Express onboarding", status: "wired", citation: "junior-backend/app/routes/stripe_connect.py:115", note: "Real stripe.Account + AccountLink; idempotent; webhook refresh." },
  ...
  { id: "mo-16", cluster: "money", name: "Sponsored campaign submission (Uncle Daniel lanes)", status: "demo", citation: "desktop-2/src/design-os/campaigns/CampaignPageShell.tsx", note: "Opens Whop URL in webview; LC polls /whop/submissions/{id}." },
  ...
  { id: "ag-18", cluster: "agency", name: "Community: chat panel (WhopChat)", status: "demo", citation: "junior-backend/app/agents/whop_chat.py", note: "Fleet framework ready; WHOP_AGENT_ENABLED=false by default." },
  { id: "ag-19", cluster: "agency", name: "Community: post message", status: "demo", citation: "whop_chat.py proxies", note: "Proxies to Whop channel_id; no local message table." },
  { id: "ag-20", cluster: "agency", name: "Notifications inbox UI", status: "demo", citation: "backend routes exist, no desktop surface", note: "GET /notifications works; desktop has no inbox render." },
  { id: "ag-21", cluster: "agency", name: "Announcements", status: "demo", citation: "CommunityChannel.is_admin_only flag exists", note: "Model supports flag; Whop handles posting." },
  ...
  { id: "ag-23", cluster: "agency", name: "Agency-preview gate (see-first-then-upgrade)", status: "demo", citation: "desktop-2/src/components/paywall/AgencyPreviewBanner.tsx", note: "Banner component exists; flow half-wired." },
  { id: "ag-24", cluster: "agency", name: "Boost pack purchase gate", status: "demo", citation: "useTierCaps.monthlyPosts", note: "Tier gate exists; purchase surface missing." },
