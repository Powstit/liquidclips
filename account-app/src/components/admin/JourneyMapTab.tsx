...
const JOURNEYS: Journey[] = [
  ...
  { id: "cp-10", cluster: "pipeline", name: "Real MP4 export (C1-T3)", status: "wired", citation: "desktop-2/src/design-os/engine/sidecar-stub.ts:exportClip", note: "libx264/veryfast/crf20/yuv420p/aac160k/+faststart · progress via -progress pipe:2. Wrapped in Watchdog pipeline/cp-10/export-clip (2026-07-06) — watchdogWrap on exportApi.exportClip · MONEY MOMENT · failures register a FailureRecord for HQ Admin." },
  { id: "cp-11", cluster: "pipeline", name: "Save-copy-as", status: "wired", citation: "desktop-2/src/design-os/engine/sidecar-stub.ts:saveCopyAs", note: "Copies MP4 to caller-supplied dest with atomic replace. Wrapped in Watchdog pipeline/cp-11/save-copy-as (2026-07-06) — watchdogWrap on exportApi.saveCopyAs · tri-state return preserved (dest/reason/error)." },
  { id: "cp-12", cluster: "pipeline", name: "Reveal-in-Finder", status: "wired", citation: "desktop-2/src/design-os/engine/sidecar-stub.ts:revealInFinder", note: "macOS open -R / Windows explorer /select / Linux xdg-open. Wrapped in Watchdog pipeline/cp-12/reveal-in-finder (2026-07-06) — watchdogWrap on exportApi.revealInFinder · tri-state return preserved." },
  { id: "cp-13", cluster: "pipeline", name: "Export history persistence", status: "wired", citation: "desktop-2/src/design-os/engine/sidecar-stub.ts:listHistory", note: "~/LiquidClips/export_history.json · atomic tmp+replace · trim to 200 rows. Wrapped in Watchdog pipeline/cp-13/export-history (2026-07-06) — watchdogWrap on exportApi.listHistory." },
  ...
  { id: "cp-15", cluster: "pipeline", name: "Import via in-app browser (spider capture)", status: "demo", citation: "desktop-2/src/sections/browse/BrowseSection.tsx:14", note: "Webview works; no auto-capture-to-clip path. Wrapped in Watchdog pipeline/cp-15/browse-in-app (2026-07-06) — React boundary around BrowseSection body so a launcher-CTA or hud-card crash renders KadeRepairScreen." },
